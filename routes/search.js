...
router.get('/:searchTerm', function(req, res) {
    ...
    const queryStatement = "for $resource in collection('/db/transformations')//tei:TEI[ft:query(.," + "'" + search + "')] return  ( <title> {$resource//tei:title/text()} </title>, <author> {$resource//tei:author/text()} </author>, <idno> {$resource//tei:idno/text()} </idno>,kwic:summarize($resource, <config width='100'/>))";
